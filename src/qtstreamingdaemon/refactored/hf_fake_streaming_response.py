"""
HfFakeStreamingResponse - Simulazione streaming HuggingFace

Basato su GptFakeStreamingResponse - formato IDENTICO (OpenAI-compatible).

FORMATO HF:
- HuggingFace usa formato OpenAI-compatible SSE
- Identico a GPT: choices[0].delta.content/tool_calls
...
class HfFakeStreamingResponse:
    """
    Simula requests.Response.iter_lines() per HuggingFace testing senza API calls.
    ...
    def __init__(self, config: Dict = None):
        ...
        # Generate unique IDs
        self.chat_id = f"hf-{uuid.uuid4().hex[:12]}"
