...
# ============================================================================
# SPECIALIZED TRANSLATORS - Command Translation Strategies
# ============================================================================

class SimpleTranslator:
    """
    Simple 1:1 command translations (< 20 lines each).

    COMMANDS:
    pwd, ps, chmod, chown, df, true, false, whoami, hostname, which,
    sleep, cd, basename, dirname, kill, mkdir, mv, yes, env, printenv, export
...
class PipelineTranslator:
    """
    Pipeline/redirect command translations (20-100 lines each).

    COMMANDS:
    ls, cat, echo, grep, head, tail, wc, sort, uniq, cut, tr,
    diff, tee, seq, file, stat, readlink, realpath, touch, du, date,
    sha256sum, sha1sum, md5sum, base64, rm, cp, mv
...
class EmulativeTranslator:
    """
    Complex command emulations (> 100 lines each).

    COMMANDS:
    find, awk, sed, test, ln, strings, watch, paste, comm, join,
    jq, curl, wget, tar, gzip, gunzip, zip, unzip, hexdump,
    timeout, split, column
    ...
    def __init__(self):
        """Initialize EmulativeTranslator"""
        pass
...
class CommandTranslator:
    ...
    Supports 70+ commands with full parameter handling

    ARCHITECTURE:
    CommandTranslator is now a DISPATCHER that delegates to specialized translators:
    - SimpleTranslator: 1:1 translations (pwd, cd, mkdir, etc.)
    - PipelineTranslator: pipeline commands (ls, cat, echo, grep, etc.)
    - EmulativeTranslator: complex emulations (find, awk, sed, etc.)

    NOTE: path_translator parameter is LEGACY and NOT USED!
    Path translation happens in BashToolExecutor.execute() BEFORE
    commands reach this layer.
    """

    def __init__(self, path_translator: PathTranslator = None, git_bash_exe=None,
                 scratch_dir=None, logger=None, command_executor=None, claude_home_unix="/home/claude"):
        # NOTE: path_translator not used, kept for backward compatibility only
        self.path_translator = path_translator

        # Dependencies for preprocessing methods
        ...
        # Instantiate specialized translators
        self.simple = SimpleTranslator()
        self.pipeline = PipelineTranslator()
        self.emulative = EmulativeTranslator()

        # Command map - delegates to specialized translators
        self.command_map = {
            # ===== SIMPLE TRANSLATOR (1:1 translations) =====
            'pwd': self.simple._translate_pwd,
            'ps': self.simple._translate_ps,
            'chmod': self.simple._translate_chmod,
            'chown': self.simple._translate_chown,
            'df': self.simple._translate_df,
            'true': self.simple._translate_true,
            'false': self.simple._translate_false,
            'whoami': self.simple._translate_whoami,
            'hostname': self.simple._translate_hostname,
            'which': self.simple._translate_which,
            'sleep': self.simple._translate_sleep,
            'cd': self.simple._translate_cd,
            'basename': self.simple._translate_basename,
            'dirname': self.simple._translate_dirname,
            'kill': self.simple._translate_kill,
            'mkdir': self.simple._translate_mkdir,
            'mv': self.simple._translate_mv,
            'yes': self.simple._translate_yes,
            'env': self.simple._translate_env,
            'printenv': self.simple._translate_printenv,
            'export': self.simple._translate_export,

            # ===== PIPELINE TRANSLATOR (pipeline/redirect) =====
            'touch': self.pipeline._translate_touch,
            'echo': self.pipeline._translate_echo,
            'wc': self.pipeline._translate_wc,
            'du': self.pipeline._translate_du,
            'date': self.pipeline._translate_date,
            'head': self.pipeline._translate_head,
            'tail': self.pipeline._translate_tail,
            'rm': self.pipeline._translate_rm,
            'cat': self.pipeline._translate_cat,
            'cp': self.pipeline._translate_cp,
            'ls': self.pipeline._translate_ls,
            'tee': self.pipeline._translate_tee,
            'seq': self.pipeline._translate_seq,
            'file': self.pipeline._translate_file,
            'stat': self.pipeline._translate_stat,
            'readlink': self.pipeline._translate_readlink,
            'realpath': self.pipeline._translate_realpath,
            'tr': self.pipeline._translate_tr,
            'sha256sum': self.pipeline._translate_sha256sum,
            'sha1sum': self.pipeline._translate_sha1sum,
            'md5sum': self.pipeline._translate_md5sum,
            'base64': self.pipeline._translate_base64,

            # ===== EMULATIVE TRANSLATOR (complex emulations) =====
            'wget': self.emulative._translate_wget,
            'curl': self.emulative._translate_curl,
            'sed': self.emulative._translate_sed,
            'diff': self.emulative._translate_diff,
            'jq': self.emulative._translate_jq,
            'awk': self.emulative._translate_awk,
            'split': self.emulative._translate_split,
            'sort': self.emulative._translate_sort,
            'uniq': self.emulative._translate_uniq,
            'join': self.emulative._translate_join,
            'hexdump': self.emulative._translate_hexdump,
            'ln': self.emulative._translate_ln,
            'grep': self.emulative._translate_grep,
            'gzip': self.emulative._translate_gzip,
            'gunzip': self.emulative._translate_gunzip,
            'timeout': self.emulative._translate_timeout,
            'tar': self.emulative._translate_tar,
            'cut': self.emulative._translate_cut,
            'strings': self.emulative._translate_strings,
            'column': self.emulative._translate_column,
            'watch': self.emulative._translate_watch,
            'paste': self.emulative._translate_paste,
            'comm': self.emulative._translate_comm,
            'zip': self.emulative._translate_zip,
            'unzip': self.emulative._translate_unzip,
            'find': self.emulative._translate_find,
            'test': self.emulative._translate_test,
        }
    ...
    def execute_command(self, command: str, timeout: int, cwd: Path, env: dict) -> Tuple[subprocess.CompletedProcess, str, str, List[Path]]:
        """
        Execute command with full preprocessing, translation, and execution.

        This is the main entry point called by BashToolExecutor.

        Args:
            command: Unix command with Windows paths (already translated by BashToolExecutor)
            timeout: Execution timeout in seconds
            cwd: Working directory
            env: Environment variables

        Returns:
            (result, translated_cmd, method, temp_files)
            temp_files: List of temporary files created during preprocessing that need cleanup
        """
        temp_files = []
        ...
        try:
            ...
            # STEP 2: Translation
            translated_cmd, use_shell, method = self.translate(command)

            # STEP 3: Execution via CommandExecutor
            result, final_translated_cmd, exec_method = self.command_executor.execute(
            ...
            return result, final_translated_cmd, exec_method, temp_files

        except Exception as e:
            # On error, still return temp_files for cleanup
            # Re-raise the exception after
            raise
...
EXPAND_DELIMITER'''
    ...
    def _process_substitution(self, command: str) -> Tuple[str, List[Path]]:
        ...
        def replace_input_substitution(match):
            ...
            try:
                ...
                # Translate command
                translated, _, _ = self.translate(cmd)
    ...
    def _translate_substitution_content(self, content: str) -> str:
        ...
        translated, use_shell, method = self.translate(content, force_translate=True)
