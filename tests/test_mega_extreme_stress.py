...
Testing patterns that Claude actually uses in production:
- Complex git operations with grep/sed/awk chains
- Multi-level heredocs with variable expansion
- Nested loops with process substitution
- Find with complex -exec and pipes
...
def test(name, cmd):
    ...
    try:
        ...
        print(f"✅ PASS - {len(result)} chars output")
        return True
...
results.append(test(
    ...
    """find . -type f \\( -name "*.py" -o -name "*.sh" \\) ! -path "*__pycache__*" ! -path "*.git*" -mtime -7 -size +1k -exec sh -c 'echo "File: {} - Lines: $(wc -l < "{}")"' \\;"""
))
...
- Total Lines of Code: $(find . -name "*.py" -exec wc -l {} \\; | awk '{sum+=$1} END {print sum}')
- Modified in last 24h: $(find . -name "*.py" -mtime -1 | wc -l)
...
results.append(test(
    ...
    '''find . -name "*.py" -type f | head -5 | while IFS= read -r file; do
...
results.append(test(
    ...
    '''diff <(find src -name "*.py" | sort) <(git ls-files src | grep "\\.py$" | sort) | head -20'''
))
...
results.append(test(
    ...
    '''total_lines=$(find . -name "*.py" -exec sh -c 'echo $(($(wc -l < "$1") + $(grep -c "^#" "$1")))' _ {} \\; | awk '{sum+=$1} END {print sum}'); echo "Total: $total_lines"'''
))
...
results.append(test(
    ...
    """find . -name "test_*.py" | head -5 | xargs -I {} sh -c 'echo "=== {} ==="; head -10 "{}" | grep -E "(def|class)" | nl'"""
))
...
results.append(test(
    ...
    """find . -name "*.py" -exec wc -l {} \\; | awk 'BEGIN{total=0; count=0; max=0; min=999999} {total+=$1; count++; if($1>max) max=$1; if($1<min) min=$1} END{print "Total:"total" Avg:"int(total/count)" Max:"max" Min:"min}'"""
))
...
results.append(test(
    ...
    '''grep -r "def " src/ | grep -v "__" | grep -E "def [a-z_]+\\(" | sed "s/.*def //" | cut -d"(" -f1 | sort | uniq -c | sort -rn | head -10'''
))
...
results.append(test(
  ...
  find . -name "*.py" -type f ! -path "*test*" | head -3 | while read f; do
    lines=$(wc -l < "$f" || echo 0)
...
if passed == total:
    print("\n🎉 ALL MEGA EXTREME TESTS PASSED! System handles real-world Claude patterns! 🎉")
else:
