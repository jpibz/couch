...
class ExecuteUnixSingleCommand:
    ...
    - Use CommandEmulator for translation
    - Delegate execution to ExecutionEngine (injected)
    - Manage priority chain with fallbacks

    NOT responsible for:
    - Analyzing pipeline structure (uses PipelineStrategy)
    - Managing subprocess (uses ExecutionEngine)
    - Path translation (uses PathTranslator)
    - Control structures conversion (CommandExecutor responsibility)
    """
    ...
    def __init__(self, logger: logging.Logger = None,
        ...
        SIMPLIFIED DESIGN:
        - Only 2 parameters (logger + test_mode)
        - Single dependency: CommandEmulator
        - ExecutionEngine injected at runtime
    ...
    def execute_single(self,
                      ...
                      git_bash_exe: Optional[str] = None) -> Tuple[str, bool]:
        """
        ...
        1. CommandEmulator translation (Simple/Medium/Complex)
           → Try emulate_command() first
           → Return (translated_cmd, True) for PowerShell

        2. Native Binary (if available and no translation)
           → Check execution_engine.is_available(cmd_name)
           → Return (command, False) for direct execution

        3. Bash Passthrough (BASH_EXE_PREFERRED or fallback)
           → If cmd in BASH_EXE_PREFERRED and bash available
           → Return (command, False) for bash execution

        4. Pass-through as-is (ultimo resort)
           → Return (command, False) unchanged
        ...
        Example Flow:
            ...
            1. Try CommandEmulator.emulate_command()
               → command_map['grep'] exists
               → _translate_grep() → PowerShell Select-String
               → Return ("Get-ChildItem -Recurse | Select-String 'pattern'", True)

            2. If translation returns None/fails, try Native Binary
               → execution_engine.is_available('grep')
               → If True: Return ("grep -r pattern .", False)

            3. If native not available, try Bash
               → If git_bash_exe exists: Return (command, False)
        ...
        # ================================================================
        # PRIORITY 1: CommandEmulator Translation
        # ================================================================
        try:
            translated = self.command_emulator.emulate_command(command)
            if translated and translated != command:
                # Translation successful
                self.logger.debug(f"CommandEmulator translated {cmd_name}")
                return translated, True  # Use PowerShell
        except Exception as e:
            self.logger.debug(f"CommandEmulator translation failed for {cmd_name}: {e}")

        # ================================================================
        # PRIORITY 2: Native Binary (best performance)
        # ================================================================
        if execution_engine and execution_engine.is_available(cmd_name):
            self.logger.debug(f"Using native binary for {cmd_name}")
            return command, False  # Pass through to native binary

        # ================================================================
        # PRIORITY 3: Bash Passthrough (BASH_EXE_PREFERRED)
        # ================================================================
        if cmd_name in self.BASH_EXE_PREFERRED and git_bash_exe:
            self.logger.debug(f"Using Git Bash for {cmd_name} (PREFERRED)")
            return command, False  # Pass through to bash

        # ================================================================
        # PRIORITY 4: Bash Passthrough Fallback
        # ================================================================
        if git_bash_exe:
        ...
        # ================================================================
        self.logger.warning(f"No translation available for {cmd_name} - passing through")
